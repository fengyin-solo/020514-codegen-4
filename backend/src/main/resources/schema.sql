...
CREATE TABLE IF NOT EXISTS food_store (
    ...
    longitude DOUBLE,
    latitude DOUBLE,
    order_paused INT DEFAULT 0 COMMENT '是否暂停接单：0正常 1暂停',
    pause_reason VARCHAR(255) COMMENT '暂停接单原因',
    resume_time DATETIME COMMENT '预计恢复接单时间',
    pause_time DATETIME COMMENT '暂停接单操作时间',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS order_info (
    ...
    check_in_date DATE,
    check_out_date DATE,
    store_id BIGINT COMMENT '自取订单所属门店（美食订单）',
    queue_no INT COMMENT '自取排队叫号（美食订单）',
    accept_time DATETIME COMMENT '门店接单时间（排队中->制作中）',
    ready_time DATETIME COMMENT '出餐时间（制作中->待取餐）',
    complete_time DATETIME COMMENT '用户确认取餐时间（待取餐->已完成）',
    void_time DATETIME COMMENT '作废时间（超时未取等）',
    ...
    INDEX idx_queue (order_type, target_id, status, queue_no)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 自取订单状态流转记录（排队中->制作中->待取餐->已完成/已作废）
CREATE TABLE IF NOT EXISTS order_status_log (
    ...
    remark VARCHAR(500),
    operator VARCHAR(50) COMMENT '操作方：USER/ADMIN/SYSTEM',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
